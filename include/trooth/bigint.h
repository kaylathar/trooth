 ...
 * @param env Environment to allocate within - this will determine environment all subsequent derived big ints are in - it is illegal to combine integer in multiple environments in the same operation.
 * @return Allocated empty big integer
 */
TR_BigInt* TR_BigInt_alloc(TR_Environment* env);

/**
 * Frees memory used by a BigInt object - <b>you are responsible for freeing any objects that result from your calculations unless the API specifies otherwise</b>
 */
...
/**
 * Frees a given division result - will also free all objects it owns <b>note that freeing internal opaque objects within a division result is invalid</b>
 */
 ...
 * @param env The environment to place this big int in - this environment will be used for any calculations this big int is used in as well - using integers from multiple environments in the same operation is not permitted.
 ...
 * @return -1 if operand1 < operand2, 0 if operand1 == operand2, 1 if opreand1 > operand2
 */
...
/**
 * Returns remainder of a division result - <b>the returned remainder remains owned by the result object, do not free it</b>
 */
TR_BigInt* TR_BigInt_DivisionResult_remainder(TR_BigInt_DivisionResult* result);

/**
 * Returns quotient of a division result - <b>the returned remainder remains owned by the result object, do not free it</b>
 */
